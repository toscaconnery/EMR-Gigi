<!-- jquery
    ============================================ -->
<script src="theme/js/vendor/jquery-1.11.3.min.js"></script>
<!-- bootstrap JS
    ============================================ -->
<script src="theme/js/bootstrap.min.js"></script>
<!-- meanmenu JS
    ============================================ -->
<script src="theme/js/jquery.meanmenu.js"></script>
<!-- mCustomScrollbar JS
    ============================================ -->
<script src="theme/js/jquery.mCustomScrollbar.concat.min.js"></script>
<!-- sticky JS
    ============================================ -->
<script src="theme/js/jquery.sticky.js"></script>
<!-- scrollUp JS
    ============================================ -->
<script src="theme/js/jquery.scrollUp.min.js"></script>
<!-- scrollUp JS
    ============================================ -->
<script src="theme/js/wow/wow.min.js"></script>
<!-- counterup JS
    ============================================ -->
<script src="theme/js/counterup/jquery.counterup.min.js"></script>
<script src="theme/js/counterup/waypoints.min.js"></script>
<script src="theme/js/counterup/counterup-active.js"></script>
<!-- jvectormap JS
    ============================================ -->
<script src="theme/js/jvectormap/jquery-jvectormap-2.0.2.min.js"></script>
<script src="theme/js/jvectormap/jquery-jvectormap-world-mill-en.js"></script>
<script src="theme/js/jvectormap/jvectormap-active.js"></script>
<!-- peity JS
    ============================================ -->
<script src="theme/js/peity/jquery.peity.min.js"></script>
<script src="theme/js/peity/peity-active.js"></script>
<!-- sparkline JS
    ============================================ -->
<script src="theme/js/sparkline/jquery.sparkline.min.js"></script>
<script src="theme/js/sparkline/sparkline-active.js"></script>
<!-- flot JS
    ============================================ -->
<script src="theme/js/flot/jquery.flot.js"></script>
<script src="theme/js/flot/jquery.flot.tooltip.min.js"></script>
<script src="theme/js/flot/jquery.flot.spline.js"></script>
<script src="theme/js/flot/jquery.flot.resize.js"></script>
<script src="theme/js/flot/jquery.flot.pie.js"></script>
<script src="theme/js/flot/jquery.flot.symbol.js"></script>
<script src="theme/js/flot/jquery.flot.time.js"></script>
<script src="theme/js/flot/dashtwo-flot-active.js"></script>
<!-- data table JS
    ============================================ -->
<script src="theme/js/data-table/bootstrap-table.js"></script>
<script src="theme/js/data-table/tableExport.js"></script>
<script src="theme/js/data-table/data-table-active.js"></script>
<script src="theme/js/data-table/bootstrap-table-editable.js"></script>
<script src="theme/js/data-table/bootstrap-editable.js"></script>
<script src="theme/js/data-table/bootstrap-table-resizable.js"></script>
<script src="theme/js/data-table/colResizable-1.5.source.js"></script>
<script src="theme/js/data-table/bootstrap-table-export.js"></script>

<!-- datapicker CSS
    ============================================ -->
<script src="theme/js/datapicker/bootstrap-datepicker.js"></script>
<script src="theme/js/datapicker/datepicker-active.js"></script>


<!-- main JS
    ============================================ -->
<script src="theme/js/main.js"></script>


<!-- notification JS
    ============================================ -->
<script src="theme/js/Lobibox.js"></script>
<script src="theme/js/notification-active.js"></script>

<script src="https://unpkg.com/axios/dist/axios.min.js"></script>